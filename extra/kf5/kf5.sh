...
do
    case "$1" in
        -h | --help)
            echo "Automatic building of KDE Framework 5"
            echo
...
for package in $(grep -v '^#' kf5.list | awk '{print $2}')
do
	...
	if [[ -z "$cont" || "$package" == "$cont" ]]; then
		...
		packagedir=$(echo $packagedir | sed -e 's/-\([0-9]\.[0-9][0-9]\.[0-9]\)/_\1/g')    # separate name and version by '_' instead of '-'
		...
		sed "s/packagename/${pkgname}/" kf5.proto \
